...
◆ **Process flows — implemented (autopilot, 8 checkpoints); user guide + skill authoring mode are deferred follow-ons.** The CLI (`ignatius flow <name>`, `validate` integration), graph viewer (DFD shapes, drill-down), process dictionary, and all 11 `flow.*` validation rules are built and tested against fixtures. Not yet covered: a `docs/guides/` how-to and a `noorm-modeling` skill mode for authoring flow markdown. Live `ignatius serve` flow routes (`/flow/<name>`, `/api/flow/<name>`) are also deferred — the static `ignatius flow` output path is the v1 surface. See `docs/spec/process-flows.md` Non-goals + the design's Open questions for the deferred set (queue-payload validation, sub-DFD position persistence, usage index).
