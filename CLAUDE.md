...
| SSADM process flows (DFD): parse, 11 `flow.*` rules, graph/dict render, `ignatius flow` CLI, recursive data-level balancing, client-side drill-down, separate-key persistence ◆ | process-flows | process-flows (+ research `ssadm-dfd-rules`) | — | — |

◆ **Process flows — implemented and first-class.** Path-first CLI (`ignatius flow [path] -o`, identical arg shape to `graph`/`dict` — a DFD is in-app navigation, not an argument), `validate` integration, process dictionary, and all 11 `flow.*` validation rules — built and tested. The **flow viewer is a custom SVG renderer** (`src/flow-view/`, separate from the ERD's Cytoscape): Gane-Sarson notation (open-ended `D#` stores, numbered process hubs, green externals), banded DFD layout (hub-and-spoke to stores/externals, no process-to-process, shared store bridging), full-bleed canvas with floating chrome (branding-block logo + breadcrumb beside it, DFD-nav card, findings panel, FAB with legend-in-menu + cross-nav, live minimap), and custom pan/zoom/drag-to-arrange with per-diagram persistence (separate `localStorage` key from the ERD). Live `serve` exposes flows hot-reloading (`/flow`, `/api/flow`, `/flow-dict`). `models/key-inherited` carries demo DFDs (`order-to-cash` with a sub-DFD + `refund`). Not yet covered: a `docs/guides/` how-to and a `noorm-modeling` skill mode for authoring flow markdown. See `docs/spec/process-flows.md` Non-goals + the design's Open questions for the deferred set (queue-payload validation, usage index).
