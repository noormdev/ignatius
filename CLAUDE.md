...
◆ **Process flows — implemented and first-class.** Path-first CLI (`ignatius flow [path] -o`, identical arg shape to `graph`/`dict` — a DFD is in-app navigation, not an argument), `validate` integration, graph viewer (DFD shapes, render-all + DFD selector, client-side drill-down), process dictionary, and all 11 `flow.*` validation rules — built and tested. Live `serve` exposes flows as a hot-reloading surface (`/flow`, `/api/flow`, `/flow-dict`) with FAB cross-nav across ERD / dict / flow / process-dict. `models/key-inherited` carries demo DFDs (`order-to-cash` with a sub-DFD + `refund`). Not yet covered: a `docs/guides/` how-to and a `noorm-modeling` skill mode for authoring flow markdown. See `docs/spec/process-flows.md` Non-goals + the design's Open questions for the deferred set (queue-payload validation, usage index).
