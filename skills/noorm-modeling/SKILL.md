...
This skill restates rules that are defined elsewhere. When any of these change, the
skill is updated by hand — the sync is explicit, never automatic (see
`docs/design/noorm-modeling-skill.md`). Read the source rather than trusting a
restatement here when the two disagree.

- `docs/spec/schema-lint-and-error-ux.md` — the linter rule catalog and its stderr output format, which `references/verification.md` parses.
- `docs/spec/derive-classification.md` — how classification and cardinality are derived from key shape, restated in `references/conventions.md`.
- `docs/spec/ignatius-project-config.md` — the `ignatius.yml` schema used by `references/templates.md`.
- `docs/design/markdown-driven-erd.md` — the entity markdown format and the IDEF1X intuition behind the conventions.
