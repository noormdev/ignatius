...
- A major version bump — ships as a minor (`feat`, no `!`, no `BREAKING CHANGE:`
  footer, so release-please cuts the next minor (0.11.0) not a major (1.0.0)).
...
- [ ] The `ignatius-modeling` skill teaches the new layout everywhere: no
  `_groups`/`_externals`/`_stores`/`flows/_*` path instruction remains in
  `skills/ignatius-modeling/**`. The model-bootstrap and entity-write steps write under
...
- [ ] Public guides `docs/guides/folder-format.md` and `docs/guides/flows.md`
  ...
  `ignatius-project-config` (design), `ignatius-modeling-skill` (design+spec),
...
| 3 | Skill: rewrite all stale path instructions to the new layout | `skills/ignatius-modeling/references/{entity-flow,model-flow,dfd-authoring,flow-templates,interviewing,reverse-engineering,templates,verification}.md`, `SKILL.md` if needed | atomic-implementer (feature) | ~8 | no `_groups`/`_externals`/`_stores`/`flows/_*` grep hit in `skills/`; bootstrap+entity+DFD steps point at new folders |
| 4 | Public docs + canonical specs/designs + CLAUDE.md + glossary | `docs/guides/folder-format.md`, `docs/guides/flows.md`, amend `docs/design/{markdown-driven-erd,ignatius-project-config,ignatius-modeling-skill,process-flows,noorm-flow-discovery}.md` + `docs/spec/{ignatius-modeling-skill,process-flows}.md`, `CLAUDE.md` feature map, `docs/glossary.md` | atomic-implementer (surgical→feature) | ~10 | no stale `_*`/per-DFD layout in guides; spec-currency clean; new feature-map row present |
...
- CP3 — `ignatius-modeling` skill rewritten across 8 reference files; `grep` for `_groups`/`_externals`/`_stores`/`flows/_` in `skills/` is zero; verified by building a model per the new instructions and validating it (`2688c0b`). Reviewer PASS, 1🔵 ("in the model root") fixed.
...
**Squashed to d024c43 — 2026-06-17.** Per-iteration SHAs above are historical (unreachable from any branch). Ships as a minor (`0.11.0`; 0.9.0 and 0.10.0 released before this work) per the user version policy.
