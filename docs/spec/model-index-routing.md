...
- SC7 — Every row carries the SHA-256 of its target. A folder's `digest` attribute hashes its row hashes. A parent's row for a child folder carries that child's digest, never the child's rows. Editing one entity file changes exactly the digests on its ancestor path and no others. A flow or sub-DFD folder row's Description is the `description:` frontmatter of that folder's own index file; when present, the row's hash is `folderDigest([childDigest, description])`, so rewording it changes the parent's digest, and when absent the row carries the child digest unchanged.
