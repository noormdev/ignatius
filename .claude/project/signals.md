...
**ModelIndex ([`src/model-index.ts`](../../src/model-index.ts), 222L):** exports `buildModelIndex(model: Model): ModelIndex` and `endpointKey(source, target): string`. Pure module — no Bun/Node/DOM imports; browser-safe; same discipline as `validate.ts`. `ModelIndex` carries 13 maps:
...
**ELK cost scaling by node count (render-perf-indexing L2):** `initFlowGraphCore` / the cy-init path scales ELK thoroughness via a node-count ladder — `<50` nodes: full-quality settings (unchanged); `50–100`: thoroughness 20; `100–200`: thoroughness 14; `200+`: thoroughness 7. The organic/stress layout also scales `arrangeOrganic` iteration counts by tier. Small models (<50 nodes) retain today's exact high-quality ELK options with zero visual regression.

**Preset-layout cache-skip (render-perf-indexing L1):** on a repeat graph load whose `layoutKey` matches a saved position set in `layout-store`, cy is constructed with `layout: { name: 'preset' }` and saved positions applied — ELK does not run. Subtype compound parents (`_cluster_*`) and joiners are sized correctly in preset mode.

**ModelIndex wiring:** `buildModelIndex(model)` is called once per Model in `App.tsx` after every model-changed event and on initial load. Per-click entity resolution (`openEntityById`, cytoscape tap, hash restore), Dictionary `nodesByGroup` scans, cluster-wiring element lookup, and error-badge lookups all use index maps rather than `.find`/`.filter`/`.includes` over arrays.

**Edge-paint texture-cache fix:** a texture-cache invalidation bug caused edges to be invisible on initial load in certain Cytoscape versions; fixed by clearing the texture cache after cy initialization completes.
