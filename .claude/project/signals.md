...
| Language | LOC | Files | % |
|----------|-----|-------|---|
| TypeScript | 50341 | 236 | 69% |
| Markdown | 18319 | 283 | 25% |
| CSS | 2583 | 2 | 3% |
| YAML | 1337 | 13 | 1% |
| Shell | 116 | 1 | 0% |
| JSON | 102 | 4 | 0% |
...
- `SHORT_LABEL_MAX = 22` — max character length for an inline canvas chip. Labels longer than this (including `db:` column lists) are rendered on hover/click, not inline (C13).
...
[`src/flow-view/flow-layout.ts`](../../src/flow-view/flow-layout.ts) (460L) — renderer-agnostic DFD layout helpers. Exports `assignStoreNumbers`, `buildFlowData`, `computeFlowLayout`, `FlowElementData`, `NodePos`. **Role-split node model (dfd-overhaul):** `buildFlowData` applies the role-split strategy — `buildExternalRouting` caps each external at at most TWO aggregated layout node copies (source copy `ext:<id>--src` in band 0 aggregates all source-role partners; sink copy `ext:<id>--snk` in band 4 aggregates all sink-role partners; single-role externals get one copy). `FlowElementData` node variant carries `extKind?: FlowKindKey` and `storeKind?: FlowKindKey` for kind-colored fills.

[`src/flow-view/FlowDiagramSvg.tsx`](../../src/flow-view/FlowDiagramSvg.tsx) (1573L) — SVG renderer consuming ELK positions and routed edge geometry. New props added by dfd-overhaul:
...
- Channel-placed, length-gated inline labels: labels satisfying `isInlineLabel` are placed in the inter-band channel; longer labels (including `db:` column lists) appear only on hover/click.
...
[`src/app/styles.css`](../../src/app/styles.css) — full SPA stylesheet. Key additions: `@media print` block (CP10), `::highlight(dd-search-highlight)` (CP9), `.dict-process-direction` badges, `.flow-minimap-wrapper` (left: 16px, CP19), `.zoom-control` (CP22/CP23), `.kbd-hint` (small keyboard-shortcut badge displayed inside FAB items and the DD lens toggle). DD chrome: `.dict-view` (full-width scroll container), `.dict-view-inner`, `.dict-search-bar` (fixed frosted top bar), `.dict-browse-lens`, `.dict-grid-card`, `.dict-flow-grid-card`, `.spotlight-overlay`, `.spotlight-line`, `.spotlight-line--flow`, `.spotlight-chips-container`, chip sub-classes, `.dict-browse-focus-bar`.
