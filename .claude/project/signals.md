...
- **Entity components:**
  ...
  - [`src/app/components/entity/GridCard.tsx`](../../src/app/components/entity/GridCard.tsx) — exports `GridCard`. Compact entity card for the DD browse lens: group-color left-border accent, name, classification badge, PK column list, column count, ⓘ affordance. Props include `spotlitClass` (CSS modifier for dim/spotlit), `onMouseEnter`/`onMouseLeave`/`onClick` carrying entity id. ⓘ click stops propagation so it never triggers spotlight pin. Used by `DictionaryView` browse lens.
  - [`src/app/components/entity/FlowNodeGridCard.tsx`](../../src/app/components/entity/FlowNodeGridCard.tsx) — exports three compact flow-node cards for the DD browse lens: `ProcessGridCard` (dotted number + label, `proc:<id>` token, process accent border), `ExternalGridCard` (`ext:<id>` token, kind-palette border color from `external.kind ?? 'external'`), `StoreGridCard` (`<kind>:<name>` token, kind-palette border color). All accept `spotlitClass`, `onOpenNode`/`onMouseEnter`/`onMouseLeave`/`onClick` carrying token string.
  - [`src/app/components/entity/SpotlightOverlay.tsx`](../../src/app/components/entity/SpotlightOverlay.tsx) — exports `SpotlightOverlay`. Position:fixed SVG + chips container rendered over the browse grid. Props: `activeId`, `connections: SpotlightConnection[]`, `flowConnections: FlowSpotlightConnection[]`, `labelHoverCardId: string | null`, `gridContainerRef`. FK connections draw SOLID bezier paths (`--spotlight-line-out` amber, `--spotlight-line-in` teal) with arrowheads; flow connections draw DASHED paths (`--spotlight-line-flow` purple). Anchors use facing-edge geometry: vertical anchor when |dy|>|dx|, horizontal otherwise. Off-screen connections render as chips (scroll-to + flash). Pills (predicate/cardinality for FK; payload for flow) rendered only for `labelHoverCardId` connection(s) — hover-reveal; collision-nudge applied when multiple pills share a midpoint. Absent-card guard: no chip or line for cards not in the DOM. ResizeObserver on `gridContainerRef` + window resize + scroll events on `[data-ignatius="dict-view"]` drive rAF-throttled redraws.
...
- **Views:**
  ...
  - [`src/app/views/dict/DictionaryView.tsx`](../../src/app/views/dict/DictionaryView.tsx) (720L+) — `DictionaryView` component. Keep-mounted via CSS `display:none`. Owns DD CSS Custom Highlight search (CP9), `beforeprint`/`afterprint` (CP10), DD sidebar process nesting with `compareDottedProcesses` (CP24), DD card endpoint clickability `externalIds`/`nonDbStoreNames` O(1) sets (CP25). **Browse lens (dd-spotlight-grid):** lens toggle (`'read' | 'browse'`) persisted to `localStorage` via `LENS_STORAGE_KEY`. Browse lens renders a `.dict-browse-lens` grid: entity groups (GridCard per node), plus Processes / External Entities / Data Stores sections (ProcessGridCard / ExternalGridCard / StoreGridCard). All stores+externals collected recursively from `allDiagrams` via useMemo (deduped). Spotlight state: `hoverId`, `pinnedId` (pin on click, Esc clears pin or focus first), `labelHoverCardId` (hover-reveal pills on connected cards only), `focusId` (focus mode: CP15 — grid filtered to `{focusId} ∪ connected-card-set`, Esc clears, search clears). Active spotlight = `pinnedId ?? hoverId`. FK connections: `buildSpotlightConnections(modelIndex, activeId)` (entity cards only). Flow lookup token: entity → `"db:<entityId>"`; flow-node → its own raw `"<kind>:<name>"` token. Flow connections: `buildFlowSpotlightConnections(allDiagrams, activeFlowToken)`. `spotlitIds` Set = `{activeId} ∪ connection otherIds ∪ flow connection otherCardIds`. Card callbacks stable via `useCallback`; spotlitClass applied per-card from `spotlitIds`. `SpotlightOverlay` mounted when `activeId !== null`. Fixed top search bar (`.dict-search-bar`): always visible in browse lens; debounced 200ms commit; search + focus are mutually exclusive (search change clears focus; `activateFocus()` clears search via sentinel `focusClearingSearchRef`). `buildEntityUsageIndex` and `buildFlowDocResolver` called internally for flow-node modal opens from browse-lens ⓘ buttons (`openFlowResult` state).
...
[`src/app/styles.css`](../../src/app/styles.css) additions: `@media print` block for the Dictionary view (CP10), `::highlight(dd-search-highlight)` rule (CP9), `.dict-process-direction` / `.dict-process-direction--read/write/readwrite` direction badges, `--dd-search-highlight` + direction-badge theme vars, `.flow-minimap-wrapper` matched to `.minimap` (left: 16px when DFD nav hidden, matching DG minimap — CP19), `.zoom-control` + child rules for both view adapters (CP22/CP23). **DD chrome restructure (dd-spotlight-grid):** `.dict-view` is now a full-width scroll container (`data-ignatius="dict-view"`); `.dict-view-inner` is the max-width wrapper inside it. `.dict-search-bar` — fixed frosted top bar (backdrop-filter, `position:fixed`). `.dict-browse-lens` — browse grid container. `.dict-grid-card` — compact entity card base; `.dict-grid-card--flash` animation on scroll-to target. `.dict-flow-grid-card`, `.dict-flow-card-number`, `.dict-flow-card-kind` — flow-node card variants. `.spotlight-overlay` — position:fixed SVG (pointer-events:none). `.spotlight-line`, `.spotlight-line--flow` — SVG path classes. `.spotlight-chips-container`, `.spotlight-chip`, `.spotlight-chip-arrow`, `.spotlight-chip-name`, `.spotlight-chip-sep`, `.spotlight-chip-pred` — off-screen chip overlay. `.dict-browse-focus-bar` — focus-mode indicator bar.
