...
| validate | src/validate.ts | Pure model validator: 25 RuleIds (14 entity-domain + 11 `flow.*`) across 5 domains, two severity tiers (A=warn, B=omit); coerces invalid pk/columns to safe defaults in cleanedModel | (below) |
| flows | src/flow-parse.ts, src/flow-validate.ts, src/flow-fingerprint.ts, src/generators/flow-graph.ts, src/generators/flow-dict.ts | SSADM data flow diagrams: `parseFlows` (recursive sub-DFD discovery, `_externals/`/`_stores/`, dotted numbering, typed endpoint resolution), `validateFlows` (11 `flow.*` rules incl. recursive data-level balancing), `layoutFlowFingerprint`, static graph + dict generators; `ignatius flow` CLI subcommand; App.tsx flow render path (`initFlowGraph`, isolated DFD styles, client-side drill-down, separate-key persistence). See `docs/spec/process-flows.md`. | (new) |
