...
- [`test/fixtures/`](../../test/fixtures) — YAML fixtures and fixture model roots. All fixture model roots use the v0.10.0 folder layout (`data/`, `groups/`, `externals/`, `stores/`; no `_*` prefixes): `flows-leveling/` (servable, 4-level-deep DFD tree), `flows-model/`, `broken-flows-model/`, `broken-flow/`, `flows/`. Per-DFD `_externals`/`_stores` were collapsed to root registries (same-name collisions in `broken-flow` Shopper and `flows-leveling` User resolved to one global definition each).
...
- package.json `name` is `ignatius`, version is `0.9.0` (released via #11); the folder-model hard-cut (#16) is merged unreleased on main and ships in the next minor (`0.10.0`). The repo *directory* is still named `derek-db-generator/` — the one remaining derek reference, a known leftover.
...
**Folder model (v0.10.0, #16):** Entity discovery scans `data/**/*.md` exclusively — the old `**/*.md` + `_`-segment-skip + `flows/`-skip logic is gone. `groups/` is optional — absent means zero groups, no throw. Files outside `data/` (at the root, in `notes/`, etc.) are never parsed as entities. The five recognized top-level folders are `data/`, `flows/`, `groups/`, `externals/`, and `stores/`; everything else is free-form and ignored.
...
- `nodeSize(n: NodeElement): { width, height }` — canonical node sizing fed to ELK. Exported so tests can compute bounding-box extents consistently with the actual layout.
...
- **Logic (pure, no DOM/React):**
  ...
  - [`src/app/logic/shortcuts.ts`](../../src/app/logic/shortcuts.ts) (75L) — pure keyboard-shortcut resolver. No DOM, no React, no Bun/Node imports; browser-safe and unit-testable. Exports `resolveShortcut(e: ShortcutKeyEvent, view: ViewName, editable: boolean): ShortcutAction | null`, `ShortcutAction` discriminated union (`{ type: 'view'; view: ViewName } | { type: 'toggleLayout' } | { type: 'toggleLens' }`), and `ShortcutKeyEvent` interface. Keymap: g→graph, d→dict, f→flow (any view); l→toggleLayout (graph only); b→toggleLens (dict only). Guards: editable=true → null; any modifier key → null. Key matched on `e.key.toLowerCase()` so capslock does not block actions. Same pure-module discipline as [`src/app/logic/spotlight.ts`](../../src/app/logic/spotlight.ts). Imported by `useKeyboardShortcuts` only.
...
- **Entity components:**
  ...
  - [`src/app/components/entity/SpotlightOverlay.tsx`](../../src/app/components/entity/SpotlightOverlay.tsx) — exports `SpotlightOverlay`. Position:fixed SVG + chips container rendered over the browse grid. FK connections draw SOLID bezier paths; flow connections draw DASHED paths (`--spotlight-line-flow` purple).
...
[`src/generators/app.ts`](../../src/generators/app.ts) (121L) — the sole static HTML generator. `generateApp(model, flowModel, bundle, opts)` writes ONE unified SPA file. Injects the full union of globals: `window.__IGNATIUS_MODE__ = "static"`, `window.__MODEL__`, `window.__FLOW_MODEL__`, `window.__LAYOUT_KEY__`, `window.__FLOW_LAYOUT_KEYS__`, `window.__THEME_MODE__`. `escapeScriptClose` sanitizes injected HTML (flow body markdown can contain `</script>`).
...
- [`src/app/logic/spotlight.ts`](../../src/app/logic/spotlight.ts) imported by [`src/app/views/dict/DictionaryView.tsx`](../../src/app/views/dict/DictionaryView.tsx) only. [`src/app/logic/flow-spotlight.ts`](../../src/app/logic/flow-spotlight.ts) imported by [`src/app/views/dict/DictionaryView.tsx`](../../src/app/views/dict/DictionaryView.tsx) only. [`src/app/components/entity/SpotlightOverlay.tsx`](../../src/app/components/entity/SpotlightOverlay.tsx) imported by [`src/app/views/dict/DictionaryView.tsx`](../../src/app/views/dict/DictionaryView.tsx) only. [`src/app/components/entity/GridCard.tsx`](../../src/app/components/entity/GridCard.tsx) imported by [`src/app/views/dict/DictionaryView.tsx`](../../src/app/views/dict/DictionaryView.tsx) only. [`src/app/components/entity/FlowNodeGridCard.tsx`](../../src/app/components/entity/FlowNodeGridCard.tsx) imported by [`src/app/views/dict/DictionaryView.tsx`](../../src/app/views/dict/DictionaryView.tsx) only. All five spotlight-grid modules form a self-contained cluster: changes to any of them force changes only within the DD browse-lens feature.
