...
// --- Test (a): base IS a model root (base/ignatius.yml exists) → single ---
{
  ...
  assert(result.kind === 'single', `FAIL (a): expected single, got ${result.kind}`);
  if (result.kind === 'single') {
    assert(result.model.dir === base, `FAIL (a): dir mismatch: ${result.model.dir}`);
    assert(result.model.name === 'Root Model', `FAIL (a): name = ${result.model.name}`);
    assert(result.model.key === '', `FAIL (a): key should be '' for base-is-root, got '${result.model.key}'`);
  }
...
// --- Test (b): container with 3 child model roots → many ---
{
  ...
  assert(result.kind === 'many', `FAIL (b): expected many, got ${result.kind}`);
  if (result.kind === 'many') {
    assert(result.models.length === 3, `FAIL (b): expected 3 models, got ${result.models.length}`);
    const keys = result.models.map(m => m.key).sort();
    assert(keys[0] === 'alpha', `FAIL (b): key[0] = ${keys[0]}`);
    assert(keys[1] === 'beta', `FAIL (b): key[1] = ${keys[1]}`);
    assert(keys[2] === 'gamma', `FAIL (b): key[2] = ${keys[2]}`);
  }
...
// --- Test (c): container + --model <key> → single ---
{
  ...
  assert(result.kind === 'single', `FAIL (c): expected single, got ${result.kind}`);
  if (result.kind === 'single') {
    assert(result.model.key === 'beta', `FAIL (c): key = ${result.model.key}`);
    assert(result.model.name === 'Beta Model', `FAIL (c): name = ${result.model.name}`);
  }
...
// --- Test (d): walk-up — base is subdir inside a model root, no ignatius.yml at or below ---
{
  ...
  assert(result.kind === 'single', `FAIL (d): expected single, got ${result.kind}`);
  if (result.kind === 'single') {
    assert(result.model.dir === root, `FAIL (d): expected dir=${root}, got ${result.model.dir}`);
    assert(result.model.name === 'Enclosing Model', `FAIL (d): name = ${result.model.name}`);
  }
...
// --- Test (e): none — no ignatius.yml at, below, or above within the fixture ceiling ---
{
  ...
  assert(result.kind === 'none', `FAIL (e): expected none, got ${result.kind}`);
...
// --- Test (f): skip-list — node_modules and _prefixed dirs are NOT counted ---
{
  ...
  assert(result.kind === 'single', `FAIL (f): expected single (only 1 valid), got ${result.kind}`);
  if (result.kind === 'single') {
    assert(result.model.key === 'valid', `FAIL (f): expected key=valid, got ${result.model.key}`);
  }
...
// --- Test (g): --model key matches nothing → no-match ---
{
  ...
  assert(result.kind === 'no-match', `FAIL (g): expected no-match, got ${result.kind}`);
  if (result.kind === 'no-match') {
    assert(Array.isArray(result.available), 'FAIL (g): available should be array');
    assert(result.available.length === 3, `FAIL (g): expected 3 available, got ${result.available.length}`);
  }
...
// --- Test (h): key is relative path, not bare basename — nested same-named dirs stay distinct ---
{
  ...
  assert(result.kind === 'many', `FAIL (h): expected many, got ${result.kind}`);
  if (result.kind === 'many') {
    const keys = result.models.map(m => m.key).sort();
    // keys should be relative paths like 'group-a/models' and 'group-b/models'
    assert(keys.length === 2, `FAIL (h): expected 2 models, got ${keys.length}`);
    assert(keys[0] === 'group-a/models', `FAIL (h): key[0] = ${keys[0]}`);
    assert(keys[1] === 'group-b/models', `FAIL (h): key[1] = ${keys[1]}`);
  }
...
// --- Test (i): --model with relative path key → single ---
{
  ...
  assert(result.kind === 'single', `FAIL (i): expected single, got ${result.kind}`);
  if (result.kind === 'single') {
    assert(result.model.key === 'group-b/models', `FAIL (i): key = ${result.model.key}`);
    assert(result.model.name === 'Group B Models', `FAIL (i): name = ${result.model.name}`);
  }
